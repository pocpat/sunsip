...
const Room: React.FC<RoomProps> = ({ isPreview = false }) => {
  ...
  // Handle city image click
  const handleCityImageClick = () => {
    if (cityImageUrl && weatherData) {
      setModalImageUrl(cityImageUrl);
      setModalAltText(`${weatherData.city}, ${weatherData.country} - ${weatherData.condition}`);
      setShowImageModal(true);
    }
  };

  // Handle cocktail image click
  const handleCocktailImageClick = () => {
    if (cocktailData) {
      setModalImageUrl(cocktailData.imageUrl);
      setModalAltText(cocktailData.name);
      setShowImageModal(true);
    }
  };
    ...
    <>
      <div className="relative w-full h-full overflow-hidden font-inter"> 
        ...
        >
          ...
          {/* Cocktail positioned on the table */}
          {!isPreview && cocktailData && (
            <motion.img
              src={cocktailData.imageUrl}
              alt={cocktailData.name}
              className="absolute right-[10%] bottom-[20%] w-[16%] rounded-lg shadow-lg z-10 cursor-pointer hover:scale-110 transition-transform duration-300"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.5, duration: 0.8 }}
              onClick={handleCocktailImageClick}
        ...
        {/* Top-level transparent click-catcher for city image - FIXED POSITIONING */}
        {!isPreview && cityImageUrl && (
          <div
            className="absolute cursor-pointer z-[100] hover:bg-white/10 transition-colors"
            style={{
              top: '10%',
              left: '31%',
              width: '32%',
        ...
        {/* Top-level transparent click-catcher for cocktail image */}
        {!isPreview && cocktailData && (
          <div
            className="absolute cursor-pointer z-[80] hover:bg-white/10 transition-colors"
            style={{
              bottom: '20%',
              right: '9%',
              width: '18%',
